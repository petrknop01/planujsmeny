                ...
                {props.item.items.map((item, i) =>
                    <View
                        ...
                        <View style={{ padding: 10 }}>
                            {item.type ?
                                <Text style={{ fontWeight: "bold", paddingBottom: 5 }} numberOfLines={1} ellipsizeMode="tail" >{item.type.name}</Text> : null
                            }
                            {(props.editovatTW && !item.type) ||  (props.editovatAbsence && item.type)? 
                            <Button small onPress={() => props.onPressEdit(item)} style={{ alignSelf: "flex-end" }}><Text>Editovat</Text></Button> : null}
                        </View>
                    </View>
                )}
